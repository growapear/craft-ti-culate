...
function pullingwalls2Visualization(selector) {
  return p => {
    let gestureData;
    let numFrames = 0;

    p.preload = () => {
      gestureData = p.loadJSON('pullingwalls2.json');
    };

    p.setup = () => {
      const eltSize = getCanvasSize(selector);
      p.createCanvas(eltSize.width, eltSize.height, p.WEBGL);

      // Process landmarks and compute centroids
      for (const key in gestureData) {
        const landmarks = gestureData[key];
        numFrames += 1;
    ...
    p.draw = () => {
      ...
      let i = p.floor((p.frameCount % numFrames) / 3);
      drawHandLandmarks(gestureData[i]);
    };
...
function onReady() {
  createSketch('#sketch1', pullingwalls2Visualization);
}
