...
function pinch2(selector) {
  return p => {
    ...
    p.preload = () => {
      pinch2 = p.loadJSON('../3dhands/pinch2.json');
    };

    p.setup = () => {
      const eltSize = getCanvasSize(selector);
      p.createCanvas(eltSize.width, eltSize.height, p.WEBGL).parent('sketch1');

      // Process landmarks and compute centroids
      for (const key in pinch2) {
        const landmarks = pinch2[key];
        numFrames += 1;
    ...
    p.draw = () => {
      ...
      drawHandLandmarks(pinch2[currentFrame]);
    };
