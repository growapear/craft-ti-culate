...
function spongerefinement(selector) {
  return p => {
    let spongerefinement;
    let numFrames = 0;
    let isPlaying = true;
    let currentFrame = 0;
    ...
    p.preload = () => {
      spongerefinement = p.loadJSON('../3dhands/sponge.json');
    };
    ...
    p.setup = () => {
      ...
      for (const key in spongerefinement) {
        const landmarks = spongerefinement[key];
        numFrames += 1;
    ...
    p.draw = () => {
      ...
      drawHandLandmarks(spongerefinement[currentFrame]);
    };
...
function onReady() {
  createSketch('#sketch1', spongerefinement);
}
