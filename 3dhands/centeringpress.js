...
function centeringpress(selector) {
  return p => {
    let centeringpress;
    let numFrames = 0;
    let isPlaying = true;
    ...
    p.preload = () => {
      centeringpress = p.loadJSON('../3dhands/centeringpress.json');
    };
    ...
    p.setup = () => {
      ...
      for (const key in centeringpress) {
        const landmarks = centeringpress[key];
        numFrames += 1;
    ...
    p.draw = () => {
      ...
      drawHandLandmarks(centeringpress[currentFrame]);
    };
...
function onReady() {
  createSketch('#hand-sketch-placeholder', centeringpress);
}
