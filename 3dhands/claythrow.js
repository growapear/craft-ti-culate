...
function claythrow(selector) {
  return p => {
    ...
    p.preload = () => {
      claythrow = p.loadJSON('../3dhands/claythrow.json');
    };
    ...
    p.setup = () => {
      ...
      for (const key in claythrow) {
        const landmarks = claythrow[key];
        numFrames += 1;
    ...
    p.draw = () => {
      ...
      drawHandLandmarks(claythrow[currentFrame]);
    };
...
function onReady() {
  createSketch('#hand-sketch-placeholder', claythrow);
}
