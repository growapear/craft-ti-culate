...
function thumbopening(selector) {
  return p => {
    let thumbopening;
    let numFrames = 0;
    let isPlaying = true;
    ...
    p.preload = () => {
      thumbopening = p.loadJSON('../3dhands/thumbopening.json');
    };
    ...
    p.setup = () => {
      ...
      for (const key in thumbopening) {
        const landmarks = thumbopening[key];
        numFrames += 1;
    ...
    p.draw = () => {
      ...
      drawHandLandmarks(thumbopening[currentFrame]);
    };
...
function onReady() {
  createSketch('#hand-sketch-placeholder', thumbopening);
}
