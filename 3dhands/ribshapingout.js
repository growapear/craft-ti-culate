...
function ribshapingout(selector) {
  return p => {
    let ribshapingout;
    let numFrames = 0;
    let isPlaying = true;
    ...
    p.preload = () => {
      ribshapingout = p.loadJSON('../3dhands/ribshapingout.json');
    };
    ...
    p.setup = () => {
      ...
      for (const key in ribshapingout) {
        const landmarks = ribshapingout[key];
        numFrames += 1;
    ...
    p.draw = () => {
      ...
      drawHandLandmarks(ribshapingout[currentFrame]);
    };
...
function onReady() {
  createSketch('#hand-sketch-placeholder', ribshapingout);
}
