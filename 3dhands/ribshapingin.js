...
function ribshapingin(selector) {
  return p => {
    let ribshapingin;
    let numFrames = 0;
    let isPlaying = true;
    ...
    p.preload = () => {
      ribshapingin = p.loadJSON('../3dhands/ribshapingin.json');
    };
    ...
    p.setup = () => {
      ...
      for (const key in ribshapingin) {
        const landmarks = ribshapingin[key];
        numFrames += 1;
    ...
    p.draw = () => {
      ...
      drawHandLandmarks(ribshapingin[currentFrame]);
    };
...
function onReady() {
  createSketch('#hand-sketch-placeholder', ribshapingin);
}
