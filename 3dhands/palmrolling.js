...
function palmrolling(selector) {
  return p => {
    ...
    p.preload = () => {
      palmrolling = p.loadJSON('../3dhands/palmrolling.json');
    };

    p.setup = () => {
      const eltSize = getCanvasSize(selector);
      const canvas = p.createCanvas(eltSize.width, eltSize.height, p.WEBGL);
      canvas.parent('sketch1');
      canvas.style('width', '100%');
      canvas.style('height', '100%');

      // Process landmarks and compute centroids
      for (const key in palmrolling) {
        const landmarks = palmrolling[key];
        numFrames += 1;
    ...
    p.draw = () => {
      ...
      drawHandLandmarks(palmrolling[currentFrame]);
    };
...
function onReady() {
  createSketch('#sketch1', palmrolling);
}
