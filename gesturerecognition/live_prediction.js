...
async function loadGestureModel() {
    try {
        ...
        gestureModel = await tf.loadLayersModel('../gesturerecognition/gesture_model/model.json');
...
async function predictGesture(landmarks) {
    if (!gestureModel || !landmarks) return null;

    try {
        // Flatten landmarks to match model input format (63 values - 21 points × 3 coordinates)
        const flatLandmarks = landmarks.reduce((arr, point) => {
            arr.push(point.x, point.y, point.z);
            return arr;
        }, []);

        // Convert to tensor and reshape to match model input shape
        ...
        // Return gesture with probability
        return {
